...
#     '''
#     def __init__(self, commands, nb_cores_per_command=1, queue, walltime, cores=1, gpus=None, modules=[], cwd=os.getcwd()):
#         if queue is None:
#             raise ValueError("Queue's name must be specified.")

#         if re.match(walltime_regex, walltime) is None:
#             raise ValueError("Walltime must be in the following format: 'dd:hh:mm:ss'.")

#         if type(cores) is int:
#             raise ValueError("Number of cores available per node must be specified.")
